...
class SeoService
{
    ...
    {
        $records=[]; foreach(DB::table('products')->where('status','active')->whereNull('deleted_at')->get(['slug','updated_at']) as $row)$records[]=['loc'=>url('/product/'.$row->slug),'lastmod'=>$row->updated_at]; foreach(DB::table('categories')->where('is_active',true)->whereNull('deleted_at')->get(['slug','updated_at']) as $row)$records[]=['loc'=>url('/category/'.$row->slug),'lastmod'=>$row->updated_at]; foreach(DB::table('pages')->where('status','published')->whereNull('deleted_at')->get(['slug','updated_at']) as $row)$records[]=['loc'=>url('/page/'.$row->slug),'lastmod'=>$row->updated_at]; return $records;
    }
}
