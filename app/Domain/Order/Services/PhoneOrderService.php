...
class PhoneOrderService
{
    ...
    /** Creates an auditable phone/admin order from public product slugs and reserves stock. */
    public function create(?int $userId, array $customer, array $lines, array $options = []): Order
    {
        ...
        return DB::transaction(function () use ($userId, $customer, $lines, $options): Order {
            $resolved = collect($lines)->map(function (array $line): array {
                ...
                $price = $this->pricing->price($product, $variant, $quantity);
            ...
            $order = Order::query()->create([
                ...
                'invoice_kind' => $options['invoice_kind'] ?? 'natural',
                'billing_profile_snapshot' => $options['billing_profile_snapshot'] ?? $customer,
                'customer_note' => $options['customer_note'] ?? null,
                'internal_note' => $options['internal_note'] ?? 'سفارش تلفنی ثبت‌شده توسط ادمین',
            ]);
            foreach ($resolved as $line) {
                $order->items()->create([
                    ...
                    'unit_price' => $line['price']['final_price'],
                    'discount_total' => 0,
                    'tax_total' => $line['product']->is_taxable ? (int) round(($line['price']['final_price'] * $line['quantity']) * ((float) $line['product']->tax_rate / 100)) : 0,
                    'line_total' => $line['price']['final_price'] * $line['quantity'],
                    'snapshot' => ['slug' => $line['product']->slug, 'brand' => $line['product']->brand?->name, 'variant' => $line['variant']?->name, 'pricing' => $line['price']],
                ]);
            }
