...
class ConfigurableJsonGateway implements PaymentGateway
{
    ...
    /** Returns the configured provider key. */
    public function name(): string
    {
        return $this->gatewayName;
    }

    /** Sends a provider-configured request for the exact remaining payable amount. */
    public function request(Order $order, string $callbackUrl, ?int $amount = null): PaymentRequest
    {
        ...
        if (! $endpoint || ! $redirectTemplate) {
            return new PaymentRequest(false, message: 'تنظیمات درگاه '.$this->gatewayName.' کامل نیست.');
        }
        $payload = [
            'api_key' => $this->settings->get($base.'api_key'),
        ...
        if (! $response->successful() || ! $authority) {
            return new PaymentRequest(false, payload: $data, message: 'ایجاد تراکنش درگاه ناموفق بود.');
        }

        return new PaymentRequest(true, (string) $authority, str_replace('{authority}', (string) $authority, $redirectTemplate), $data);
    }

    /** Verifies configurable gateways with stored authority and exact transaction amount. */
    public function verify(PaymentTransaction $transaction, array $callback): PaymentVerification
    {
        ...
        if (! $endpoint) {
            return new PaymentVerification(false, message: 'آدرس Verify درگاه تنظیم نشده است.');
        }
        ...
        $referencePath = $this->settings->get($base.'reference_path', 'track_id');

        return new PaymentVerification(
            (bool) data_get($data, $successPath, false),
            (string) data_get($data, $referencePath, ''),
            $data,
            $response->successful() ? null : 'Verify HTTP failed',
        );
    }

    /** Uses an optional configurable refund endpoint and fails closed when unavailable. */
    public function refund(PaymentTransaction $transaction, int $amount): PaymentVerification
    {
        $base = 'payments.'.$this->gatewayName.'.';
        $endpoint = $this->settings->get($base.'refund_url');
        if (! $endpoint) {
            return new PaymentVerification(false, message: 'Refund برای این درگاه فعال نیست.');
        }
        $response = Http::timeout(15)->acceptJson()->post($endpoint, [
            'api_key' => $this->settings->get($base.'api_key'),
        ...
        return new PaymentVerification($response->successful(), (string) data_get($data, 'reference_id', ''), $data);
    }
}
