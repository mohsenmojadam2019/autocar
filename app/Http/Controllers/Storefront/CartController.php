...
class CartController extends Controller
{
    ...
    {
        $data=$request->validate(['quantity'=>['required','integer','min:1','max:99']]); $cart=$this->cart($request,$service);
        $line=$cart->items()->whereKey($item)->firstOrFail(); $line->update(['quantity'=>$data['quantity']]); $cart->touch();
    ...
    {
        $this->cart($request,$service)->items()->whereKey($item)->delete(); return back()->with('success','محصول از سبد حذف شد.');
    }
    ...
    {
        $cart=$service->resolve($request->user()?->id,$request->session()->get('cart_token')); $request->session()->put('cart_token',$cart->token); return $cart;
    }
}
