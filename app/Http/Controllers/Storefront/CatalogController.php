...
class CatalogController extends Controller
{
    ...
    /** Lists a slug-bound brand landing page without exposing numeric brand identifiers. */
    public function brand(Request $request, Brand $brand): View
    {
        ...
        return view('storefront.catalog.index', [
            'title' => 'قطعات '.$brand->name,
            'category' => null,
            'brand' => $brand,
            'products' => $query->paginate(24)->withQueryString(),
            'brands' => Brand::query()->visible()->orderBy('name')->get(),
            'breadcrumbs' => [['name' => 'برندها', 'url' => route('home').'#brands'], ['name' => $brand->name, 'url' => null]],
        ]);
    }
    ...
    /** Shows a slug-bound product with timed pricing, curated merchandising, ratings and questions. */
    public function product(Product $product, ProductRecommendationService $recommendations): View
    {
        ...
        $rating = DB::table('reviews')
            ...
            ->selectRaw('COUNT(*) as reviews_count, COALESCE(AVG(rating), 0) as rating_average')
            ->first();

        return view('storefront.product.show', [
            'product' => $product,
            'price' => $product->priceSnapshot(),
            'similar' => $recommendations->similar($product),
            'complementary' => $recommendations->complementary($product),
            'alternatives' => $recommendations->alternatives($product),
            'upsells' => $recommendations->upsells($product),
            ...
            'ratingAverage' => (float) ($rating?->rating_average ?? 0),
            'reviewsCount' => (int) ($rating?->reviews_count ?? 0),
        ]);
    }
    ...
    private function applyFilters(Builder|BelongsToMany $query, Request $request, bool $allowBrand = true): void
    {
        if ($allowBrand) {
            $query->when(
                $request->string('brand_slug')->toString(),
                fn ($builder, string $slug) => $builder->whereHas('brand', fn ($brand) => $brand->where('slug', $slug)),
            );
        }

        $query->when($request->integer('min_price'), fn ($builder, int $value) => $builder->where('sale_price', '>=', $value))
